...
def convert_currency(amount, from_currency, to_currency):
    # Először HUF-ba váltunk, majd a cél pénznembe
...
def main():
    ...
    # Pénznem választó a statisztikákhoz
...
def show_overview(expenses, income, display_currency):
    ...
    # Összesítések számítása a választott pénznemben
    total_expenses = 0
    ...
    with col1:
        st.metric("📤 Összes Kiadás", f"{total_expenses:,.0f} {symbol}",
                 delta_color="inverse")
    
    with col2:
        st.metric("📥 Összes Bevétel", f"{total_income:,.0f} {symbol}",
                 delta_color="normal")
    
    with col3:
        st.metric("💰 Haszon", f"{profit:,.0f} {symbol}",
    ...
    # Színes, látványos oszlopdiagram
    st.subheader("📊 Kiadás, Bevétel, Haszon Összehasonlítás")
    
    fig = go.Figure(data=[
        go.Bar(
            name='Kiadás',
            x=['Pénzügyi összesítő'],
            y=[total_expenses],
            marker=dict(
                color='#FF4444',
                line=dict(color='#CC0000', width=2)
            ),
            text=[f"{total_expenses:,.0f} {symbol}"],
            textposition='auto',
            hovertemplate='Kiadás: %{y:,.0f} ' + symbol + '<extra></extra>'
        ),
        go.Bar(
            name='Bevétel',
            x=['Pénzügyi összesítő'],
            ...
            text=[f"{total_income:,.0f} {symbol}"],
            textposition='auto',
            hovertemplate='Bevétel: %{y:,.0f} ' + symbol + '<extra></extra>'
        ),
        go.Bar(
            name='Haszon',
            x=['Pénzügyi összesítő'],
            y=[profit],
            marker=dict(
                color='#2196F3' if profit >= 0 else '#FF9800',
                line=dict(color='#1565C0' if profit >= 0 else '#E65100', width=2)
            ),
            text=[f"{profit:,.0f} {symbol}"],
            textposition='auto',
            hovertemplate='Haszon: %{y:,.0f} ' + symbol + '<extra></extra>'
        )
    ])
    
    fig.update_layout(
        title={
            'text': f'Pénzügyi Összesítő ({CURRENCY_INFO[display_currency]["name"]})',
    ...
    with col1:
        if not expenses.empty:
            ...
            expenses_copy['amount_converted'] = expenses_copy.apply(
                lambda row: convert_currency(row['amount'], row['currency'], display_currency), axis=1
            )
            ...
            fig.update_traces(
                ...
                hovertemplate='%{label}<br>%{value:,.0f} ' + symbol + '<br>%{percent}<extra></extra>',
    ...
    with col2:
        if not income.empty and not expenses.empty:
            ...
            # Gauge chart a haszonkulcshoz
            fig = go.Figure(go.Indicator(
                mode="gauge+number+delta",
                value=profit_margin,
                domain={'x': [0, 1], 'y': [0, 1]},
                title={'text': "Haszonkulcs %", 'font': {'size': 16}},
                delta={'reference': 20},
                gauge={
                    'axis': {'range': [None, 100], 'tickwidth': 1},
                    'bar': {'color': "#4CAF50" if profit_margin >= 20 else "#FFA726"},
                    'steps': [
                        {'range': [0, 10], 'color': "#ffcccc"},
                        {'range': [10, 30], 'color': "#ffffcc"},
...
def show_expenses(expenses):
    ...
    with st.expander("➕ Új Kiadás Hozzáadása", expanded=True):
        ...
        with col2:
            item = st.text_input("Megnevezés", placeholder="pl. Csíra magvak")
        
        with col3:
            amount = st.number_input("Összeg", min_value=0.0, step=100.0, format="%.0f")
        
        with col4:
            currency = st.selectbox("Pénznem", ['HUF', 'EUR', 'RSD'])
    ...
    if not expenses.empty:
        ...
        display_expenses['Összeg (Eredeti)'] = display_expenses.apply(
            lambda row: f"{row['amount']:,.0f} {CURRENCY_INFO[row['currency']]['symbol']}", axis=1
        )
        display_expenses['Összeg (HUF)'] = display_expenses.apply(
            lambda row: f"{convert_currency(row['amount'], row['currency'], 'HUF'):,.0f} Ft", axis=1
        )
        display_expenses['Kategória'] = display_expenses['category']
        
        st.dataframe(
            display_expenses[['Dátum', 'Megnevezés', 'Összeg (Eredeti)', 'Összeg (HUF)', 'Kategória']],
        ...
        # Összesítés
        total_huf = sum(
            convert_currency(row['amount'], row['currency'], 'HUF')
            for _, row in expenses.iterrows()
        )
...
def show_income(income):
    ...
    with st.expander("➕ Új Bevétel Hozzáadása", expanded=True):
        ...
        with col2:
            description = st.text_input("Megnevezés", placeholder="pl. Brokkoli csíra eladás")
        
        with col3:
            amount = st.number_input("Összeg", min_value=0.0, step=100.0, format="%.0f")
        
        with col4:
            currency = st.selectbox("Pénznem", ['HUF', 'EUR', 'RSD'], key="income_currency")
    ...
    if not income.empty:
        ...
        display_income['Összeg (Eredeti)'] = display_income.apply(
            lambda row: f"{row['amount']:,.0f} {CURRENCY_INFO[row['currency']]['symbol']}", axis=1
        )
        display_income['Összeg (HUF)'] = display_income.apply(
            lambda row: f"{convert_currency(row['amount'], row['currency'], 'HUF'):,.0f} Ft", axis=1
        )
        
        st.dataframe(
            display_income[['Dátum', 'Megnevezés', 'Összeg (Eredeti)', 'Összeg (HUF)']],
            use_container_width=True,
            hide_index=True
        )
...
def show_equipment(equipment, display_currency):
    ...
    with st.expander("➕ Új Eszköz Hozzáadása", expanded=True):
        ...
        with col1:
            name = st.text_input("Eszköz neve", placeholder="pl. Csíráztató tálca")
        
        with col2:
            purchase_date = st.date_input("Beszerzés dátuma", datetime.now())
        
        with col3:
            cost = st.number_input("Költség", min_value=0.0, step=100.0, format="%.0f")
        
        with col4:
            currency = st.selectbox("Pénznem", ['HUF', 'EUR', 'RSD'], key="equipment_currency")
    ...
    if not equipment.empty:
        ...
        display_equipment['Költség (Eredeti)'] = display_equipment.apply(
            lambda row: f"{row['cost']:,.0f} {CURRENCY_INFO[row['currency']]['symbol']}", axis=1
        )
        display_equipment['Költség (HUF)'] = display_equipment.apply(
            lambda row: f"{convert_currency(row['cost'], row['currency'], 'HUF'):,.0f} Ft", axis=1
        )
        display_equipment['Állapot'] = display_equipment['status']
        
        st.dataframe(
            display_equipment[['Név', 'Beszerzés dátuma', 'Költség (Eredeti)', 'Költség (HUF)', 'Állapot']],
            use_container_width=True,
            hide_index=True
        )
...
def show_detailed_stats(expenses, income, display_currency):
    ...
    if not expenses.empty or not income.empty:
        ...
        # Kiadások havi bontásban
        if not expenses.empty:
            expenses['date'] = pd.to_datetime(expenses['date'])
            expenses['amount_converted'] = expenses.apply(
                lambda row: convert_currency(row['amount'], row['currency'], display_currency), axis=1
            )
            expenses['month'] = expenses['date'].dt.strftime('%Y-%m')
            monthly_expenses = expenses.groupby('month')['amount_converted'].sum()
        
        # Bevételek havi bontásban
        if not income.empty:
            income['date'] = pd.to_datetime(income['date'])
            income['amount_converted'] = income.apply(
                lambda row: convert_currency(row['amount'], row['currency'], display_currency), axis=1
            )
        ...
        all_months = []
        if not expenses.empty:
            all_months.extend(expenses['month'].unique())
        if not income.empty:
            all_months.extend(income['month'].unique())
        ...
        for month in all_months:
            ...
            monthly_data.append({
                'Hónap': month,
                f'Kiadás ({symbol})': round(exp, 2),
                f'Bevétel ({symbol})': round(inc, 2),
                f'Haszon ({symbol})': round(profit, 2),
                'Haszonkulcs (%)': round(margin, 1)
            })
        
        if monthly_data:
            monthly_df = pd.DataFrame(monthly_data)
            
            # Táblázat színezéssel
            ...
            st.dataframe(
                ...
                hide_index=True,
                column_config={
                    f'Kiadás ({symbol})': st.column_config.NumberColumn(format="%.0f"),
                    f'Bevétel ({symbol})': st.column_config.NumberColumn(format="%.0f"),
                    f'Haszon ({symbol})': st.column_config.NumberColumn(format="%.0f"),
                    'Haszonkulcs (%)': st.column_config.NumberColumn(format="%.1f%%"),
                }
            )
            
            # Látványos havi trend grafikon
            ...
            fig.add_trace(go.Bar(
                name='Kiadás',
                x=monthly_df['Hónap'],
                y=monthly_df[f'Kiadás ({symbol})'],
                marker=dict(
                    color='#FF6B6B',
                    line=dict(color='#FF4444', width=1.5)
                ),
                text=[f"{x:,.0f} {symbol}" for x in monthly_df[f'Kiadás ({symbol})']],
                textposition='inside',
                hovertemplate='%{x}<br>Kiadás: %{y:,.0f} ' + symbol + '<extra></extra>'
            ))
            
            fig.add_trace(go.Bar(
                name='Bevétel',
                x=monthly_df['Hónap'],
                y=monthly_df[f'Bevétel ({symbol})'],
                marker=dict(
                    color='#51CF66',
                    line=dict(color='#2F9E44', width=1.5)
                ),
                text=[f"{x:,.0f} {symbol}" for x in monthly_df[f'Bevétel ({symbol})']],
                textposition='inside',
                hovertemplate='%{x}<br>Bevétel: %{y:,.0f} ' + symbol + '<extra></extra>'
            ))
            
            fig.add_trace(go.Bar(
                name='Haszon',
                x=monthly_df['Hónap'],
                y=monthly_df[f'Haszon ({symbol})'],
                marker=dict(
                    color=['#339AF0' if x >= 0 else '#FF922B' for x in monthly_df[f'Haszon ({symbol})']],
                    line=dict(color=['#1971C2' if x >= 0 else '#E8590C' for x in monthly_df[f'Haszon ({symbol})']], width=1.5)
                ),
                text=[f"{x:,.0f} {symbol}" for x in monthly_df[f'Haszon ({symbol})']],
                textposition='inside',
                hovertemplate='%{x}<br>Haszon: %{y:,.0f} ' + symbol + '<extra></extra>'
            ))
            ...
            fig.add_trace(go.Scatter(
                name='Haszonkulcs %',
                x=monthly_df['Hónap'],
                y=monthly_df['Haszonkulcs (%)'],
                mode='lines+markers',
                yaxis='y2',
                line=dict(color='#845EF7', width=3),
                marker=dict(size=10, color='#845EF7', line=dict(color='white', width=2)),
                hovertemplate='%{x}<br>Haszonkulcs: %{y:.1f}%<extra></extra>'
            ))
            
            fig.update_layout(
                title={
                    'text': f'Havi Pénzügyi Kimutatás ({CURRENCY_INFO[display_currency]["name"]})',
                    'font': {'size': 20, 'color': '#333333'}
                },
                barmode='group',
            ...
            # Havi összehasonlítás vízesés diagram
            ...
            fig_waterfall.update_layout(
                title=f"Havi Haszon Változása ({symbol})",
                height=400,
                plot_bgcolor='rgba(248, 249, 250, 1)',
                paper_bgcolor='rgba(255, 255, 255, 1)',
                yaxis=dict(title=f'Haszon ({symbol})')
            )
